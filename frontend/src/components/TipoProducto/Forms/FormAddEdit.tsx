...
function AddEditForm(props: { item?: ITipoProducto, addItemToState?: any, toggle?: any, updateState?: any }) {
  ...
  const submitFormAdd = (e: any) => {
    ...
    if (errores.length === 0) {
      ...
      fetch(`${URL}/api/General/TipoProducto_Insert/`, {
        ...
        .then((item: ITipoProducto) => {
          if (item.TipoProductoId > 0) {
  ...
  const submitFormEdit = (e: any) => {
    ...
    fetch(`${URL}/api/General/TipoProducto_Insert/`, {
      ...
      body: JSON.stringify({
        TipoProductoId: form.TipoProductoId,
        Nombre: form.Nombre,
        CodUsuario: "Adm",
        FechaRegistro: new Date(),
        Estado: true,
  ...
  useEffect(() => {
    if (props.item) {
      const { TipoProductoId, Nombre } = props.item;
      // setValues({ TipoProductoId, Nombre });

      setValues({
        ...form,
        TipoProductoId, Nombre
      });
    }
  }, [props.item]);
