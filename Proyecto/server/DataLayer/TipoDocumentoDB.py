...
class TipoDocumentoDB:
    def GetItems():
        try:
            conn = get_connection()
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                cursor.callproc("sp_TipoDocumentoAllItems")
                resulset = cursor.fetchall()
            conn.close()
            list = []

            for row in resulset:
                Data_ent = TipoDocumentoItemModel.Cargar(row)
    ...
    def GetItem(Id: int):
        try:
            ...
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                args = (Id,)
                cursor.callproc("sp_TipoDocumentoAllItem", args)
                resulset = cursor.fetchall()
            conn.close()
            list = []
    ...
    def Save(Ent: TipoDocumentoSaveModel):
        try:
            Store: str
            Store = "sp_TipoDocumento_Save"
            if Ent.Action == ProcessActionEnum.Update:
                Store = "sp_TipoDocumento_Update"
            conn = get_connection()
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                args = []
                ...
                args.append(Ent.EsAlmacen)
                args.append(Ent.EsFacturacion)
                cursor.callproc(Store, args)
                Ent.TipoDocumentoId = int(cursor.fetchone()["v_TipoDocumentoId"])
    ...
    def Delete(Id: int):
        try:
            ...
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                args = (Id,)
                cursor.callproc("sp_TipoDocumento_Delete", args)
                conn.commit()
