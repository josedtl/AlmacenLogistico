...
class EstadoProcesoDB:
    ...
    def Save(Ent: EstadoProcesoSaveModel):
        try:
            Store: str
            Store = "sp_EstadoProceso_Save"
            if Ent.Action == ProcessActionEnum.Update:
                Store = "sp_EstadoProceso_Update"
            conn = get_connection()
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                args = []
                args.append(Ent.EstadoProcesoId)
                args.append(Ent.Valor)
                ...
                Ent.EstadoProcesoId = int(cursor.fetchone()["v_EstadoProcesoId"])
    ...
    def Delete(Id: int):
        try:
            ...
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                args = (Id,)
                cursor.callproc("sp_EstadoProceso_Delete", args)
                conn.commit()
