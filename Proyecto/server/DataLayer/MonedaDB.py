...
class MonedaDB:
    def GetItems():
        try:
            ...
            with conn.cursor() as cursor:
                ...
                cursor.callproc("sp_MonedaAllItems")
                resulset = cursor.fetchall()
            conn.close()
            list = []

            for row in resulset:
                Data_ent = MonedaItemModel.Cargar(row)
    ...
    def GetItem(Id: int):
        try:
            ...
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                args = (Id,)
                cursor.callproc("sp_MonedaAllItem", args)
                resulset = cursor.fetchall()
            conn.close()
            list = []
    ...
    def Save(Ent: MonedaSaveModel):
        try:
            Store: str
            Store = "sp_Moneda_Save"
            if Ent.Action == ProcessActionEnum.Update:
                Store = "sp_Moneda_Update"
            conn = get_connection()
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                args = []
                args.append(Ent.MonedaId)
                args.append(Ent.CodMoneda)
                args.append(Ent.Simbolo)
                args.append(Ent.Descripcion)
                cursor.callproc(Store, args)
                Ent.MonedaId = int(cursor.fetchone()["v_MonedaId"])
    ...
    def Delete(Id: int):
        try:
            ...
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                args = (Id,)
                cursor.callproc("sp_Moneda_Delete", args)
                conn.commit()
