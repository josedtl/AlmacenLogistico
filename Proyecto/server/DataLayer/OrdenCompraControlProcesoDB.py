...
class OrdenCompraControlProcesoDB:
    def GetItems():
        try:
            conn = get_connection()
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                cursor.callproc("sp_OrdenCompraControlProcesoAllItems")
                resulset = cursor.fetchall()
            conn.close()
            list = []

            for row in resulset:
                Data_ent = OrdenCompraControlProcesoItemModel.Cargar(row)
    ...
    def GetItem(Id: int):
        try:
            ...
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                args = (Id,)
                cursor.callproc("sp_OrdenCompraControlProcesoAllItem", args)
                resulset = cursor.fetchall()
            conn.close()
            list = []
    ...
    def Save(Ent: OrdenCompraControlProcesoSaveModel):
        try:
            Store: str
            Store = "sp_OrdenCompraControlProceso_Save"
            if Ent.Action == ProcessActionEnum.Update:
                Store = "sp_OrdenCompraControlProceso_Update"
            conn = get_connection()
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                args = []
                args.append(Ent.OrdenCompraControlProcesoId)
                ...
                Ent.OrdenCompraControlProcesoId = int(cursor.fetchone()["v_OrdenCompraControlProcesoId"])
    ...
    def Delete(Id: int):
        try:
            ...
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                args = (Id,)
                cursor.callproc("sp_OrdenCompraControlProceso_Delete", args)
                conn.commit()
