...
class ClientePersonaNaturalDB:
    def GetItems():
        try:
            conn = get_connection()
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                cursor.callproc("sp_ClientePersonaNaturalAllItems")
                resulset = cursor.fetchall()
            conn.close()
            list = []

            for row in resulset:
                Data_ent = ClientePersonaNaturalItemModel.Cargar(row)
    ...
    def GetItem(Id: int):
        try:
            ...
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                args = (Id,)
                cursor.callproc("sp_ClientePersonaNaturalAllItem", args)
                resulset = cursor.fetchall()
            conn.close()
            list = []
    ...
    def Save(Ent: ClientePersonaNaturalSaveModel):
        try:
            Store: str
            Store = "sp_ClientePersonaNatural_Save"
            ...
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                args = []
                args.append(Ent.ClientePersonaNaturalId)
                args.append(Ent.ClienteId)
                args.append(Ent.PersonaNaturalId)
                cursor.callproc(Store, args)
                Ent.ClientePersonaNaturalId = int(cursor.fetchone()["v_ClientePersonaNaturalId"])
    ...
    def Delete(Id: int):
        try:
            ...
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                args = (Id,)
                cursor.callproc("sp_ClientePersonaNatural_Delete", args)
                conn.commit()
