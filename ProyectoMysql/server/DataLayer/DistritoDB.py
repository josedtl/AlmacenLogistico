...
class DistritoDB:
    def GetItems():
        try:
            ...
            with conn.cursor() as cursor:
                ...
                cursor.callproc("sp_DistritoAllItems")
                resulset = cursor.fetchall()
            conn.close()
            list = []

            for row in resulset:
                Data_ent = DistritoItemModel.Cargar(row)
    ...
    def GetItem(Id: int):
        try:
            ...
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                args = (Id,)
                cursor.callproc("sp_DistritoAllItem", args)
                resulset = cursor.fetchall()
            conn.close()
            list = []
    ...
    def Save(Ent: DistritoSaveModel):
        try:
            Store: str
            Store = "sp_Distrito_Save"
            if Ent.Action == ProcessActionEnum.Update:
                Store = "sp_Distrito_Update"
            conn = get_connection()
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                args = []
                args.append(Ent.DistritoId)
                args.append(Ent.CodDistrito)
                args.append(Ent.NomDistrito)
                args.append(Ent.ProvinciaId)
                cursor.callproc(Store, args)
                Ent.DistritoId = int(cursor.fetchone()["v_DistritoId"])
    ...
    def Delete(Id: int):
        try:
            ...
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                args = (Id,)
                cursor.callproc("sp_Distrito_Delete", args)
                conn.commit()
