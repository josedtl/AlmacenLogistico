...
class ModuloSistemaDB:
    def GetItems():
        try:
            conn = get_connection()
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                cursor.callproc("sp_ModuloSistemaAllItems")
                resulset = cursor.fetchall()
            conn.close()
            list = []

            for row in resulset:
                Data_ent = ModuloSistemaItemModel.Cargar(row)
    ...
    def GetItem(Id: int):
        try:
            ...
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                args = (Id,)
                cursor.callproc("sp_ModuloSistemaAllItem", args)
                resulset = cursor.fetchall()
            conn.close()
            list = []
    ...
    def Save(Ent: ModuloSistemaSaveModel):
        try:
            Store: str
            Store = "sp_ModuloSistema_Save"
            if Ent.Action == ProcessActionEnum.Update:
                Store = "sp_ModuloSistema_Update"
            conn = get_connection()
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                args = []
                ...
                Ent.ModuloSistemaId = int(cursor.fetchone()["v_ModuloSistemaId"])
    ...
    def Delete(Id: int):
        try:
            ...
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                args = (Id,)
                cursor.callproc("sp_ModuloSistema_Delete", args)
                conn.commit()
