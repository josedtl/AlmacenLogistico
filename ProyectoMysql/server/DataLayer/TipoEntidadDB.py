...
class TipoProcesoDB:
    def GetItems():
        try:
            ...
            with conn.cursor() as cursor:
                ...
                cursor.callproc("sp_TipoProcesoAllItems")
                resulset = cursor.fetchall()
            conn.close()
            list = []

            for row in resulset:
                Data_ent = TipoProcesoItemModel.Cargar(row)
    ...
    def GetItem(Id: int):
        try:
            ...
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                args = (Id,)
                cursor.callproc("sp_TipoProcesoAllItem", args)
                resulset = cursor.fetchall()
            conn.close()
            list = []
    ...
    def Save(Ent: TipoProcesoSaveModel):
        try:
            Store: str
            Store = "sp_TipoProceso_Save"
            if Ent.Action == ProcessActionEnum.Update:
                Store = "sp_TipoProceso_Update"
            conn = get_connection()
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                args = []
                ...
                Ent.TipoProcesoId = int(cursor.fetchone()["v_TipoProcesoId"])
    ...
    def Delete(Id: int):
        try:
            ...
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                args = (Id,)
                cursor.callproc("sp_TipoProceso_Delete", args)
                conn.commit()
