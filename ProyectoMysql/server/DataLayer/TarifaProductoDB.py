...
class TarifaProductoDB:
    def GetItems():
        try:
            conn = get_connection()
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                cursor.callproc("sp_TarifaProductoAllItems")
                resulset = cursor.fetchall()
            conn.close()
            list = []

            for row in resulset:
                Data_ent = TarifaProductoItemModel.Cargar(row)
    ...
    def GetItem(Id: int):
        try:
            ...
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                args = (Id,)
                cursor.callproc("sp_TarifaProductoAllItem", args)
                resulset = cursor.fetchall()
            conn.close()
            list = []
    ...
    def Save(Ent: TarifaProductoSaveModel):
        try:
            Store: str
            Store = "sp_TarifaProducto_Save"
            if Ent.Action == ProcessActionEnum.Update:
                Store = "sp_TarifaProducto_Update"
            conn = get_connection()
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                args = []
                args.append(Ent.TarifaProductoId)
                args.append(Ent.ProductoId)
                args.append(Ent.UnidaMedidaId)
                args.append(Ent.MonedaId)
                args.append(Ent.ValorCompra)
                args.append(Ent.ValorVenta)
                args.append(Ent.FechaRegistro)
                args.append(Ent.FechaVigencia)
                args.append(Ent.CodUsuario)
                args.append(Ent.EstadoRegistro)
                cursor.callproc(Store, args)
                Ent.TarifaProductoId = int(cursor.fetchone()["v_TarifaProductoId"])
    ...
    def Delete(Id: int):
        try:
            ...
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                args = (Id,)
                cursor.callproc("sp_TarifaProducto_Delete", args)
                conn.commit()
