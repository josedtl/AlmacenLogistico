...
class ProcesoSecuenciaDB:
    def GetItems():
        try:
            conn = get_connection()
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                cursor.callproc("sp_ProcesoSecuenciaAllItems")
                resulset = cursor.fetchall()
            conn.close()
            list = []

            for row in resulset:
                Data_ent = ProcesoSecuenciaItemModel.Cargar(row)
    ...
    def GetItem(Id: int):
        try:
            ...
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                args = (Id,)
                cursor.callproc("sp_ProcesoSecuenciaAllItem", args)
                resulset = cursor.fetchall()
            conn.close()
            list = []
    ...
    def Save(Ent: ProcesoSecuenciaSaveModel):
        try:
            Store: str
            Store = "sp_ProcesoSecuencia_Save"
            if Ent.Action == ProcessActionEnum.Update:
                Store = "sp_ProcesoSecuencia_Update"
            conn = get_connection()
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                args = []
                args.append(Ent.ProcesoSecuenciaId)
                args.append(Ent.ProcesoId)
                args.append(Ent.EstadoProcesoId)
                args.append(Ent.AnteriorEstadoProcesoId)
                args.append(Ent.OrdenProceso)
                args.append(Ent.FechaRegistro)
                args.append(Ent.CodUsuario)
                args.append(Ent.EstadoRegistro)
                cursor.callproc(Store, args)
                Ent.ProcesoSecuenciaId = int(cursor.fetchone()["v_ProcesoSecuenciaId"])
    ...
    def Delete(Id: int):
        try:
            ...
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                args = (Id,)
                cursor.callproc("sp_ProcesoSecuencia_Delete", args)
                conn.commit()
