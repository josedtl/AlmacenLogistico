...
//service
import StockMercaderiaService from '../../Service/StockMercaderiaService';
...
function Main() {
  ...
  const sStockMercaderia = new StockMercaderiaService();

  const [items, setItems] = useState<StockMercaderiaMainModel[]>([]);
  ...
  const getItems = async () => {
    const itemsg = await sStockMercaderia.GetMain();
    setItems(itemsg);
    console.log(itemsg);
    setCargarPage(false);
    ...
    <Spin spinning={CargarPage} tip="Cargando" size="large">
      <Row>
        ...
        <Col xs={18} sm={18} md={12} lg={12} xl={12}>
          <Title level={2}> StockMercaderia</Title>
        </Col>

        <Col xs={6} sm={6} md={12} lg={12} xl={12}>
          <Link to={`/StockMercaderiaSave/0`}>
            <Button
              style={ButtonAddMain}
        ...
        <Col xs={24} sm={24} md={12} lg={12} xl={12}>
          ...
          <Input
            placeholder='Buscar StockMercaderia'
            type="text"
            name="Nombre"
            onChange={onChange}
