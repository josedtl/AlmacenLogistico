...
class OrdenPedidoControlProcesoDB:
    def GetItems():
        try:
            conn = get_connection()
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                cursor.callproc("sp_OrdenPedidoControlProcesoAllItems")
                resulset = cursor.fetchall()
            conn.close()
            list = []

            for row in resulset:
                Data_ent = OrdenPedidoControlProcesoItemModel.Cargar(row)
    ...
    def GetItem(Id: int):
        try:
            ...
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                args = (Id,)
                cursor.callproc("sp_OrdenPedidoControlProcesoAllItem", args)
                resulset = cursor.fetchall()
            conn.close()
            list = []
    ...
    def Save(Ent: OrdenPedidoControlProcesoSaveModel):
        try:
            Store: str
            Store = "sp_OrdenPedidoControlProceso_Save"
            if Ent.Action == ProcessActionEnum.Update:
                Store = "sp_OrdenPedidoControlProceso_Update"
            conn = get_connection()
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                args = []
                args.append(Ent.OrdenPedidoControlProcesoId)
                ...
                Ent.OrdenPedidoControlProcesoId = int(cursor.fetchone()["v_OrdenPedidoControlProcesoId"])
    ...
    def Delete(Id: int):
        try:
            ...
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                args = (Id,)
                cursor.callproc("sp_OrdenPedidoControlProceso_Delete", args)
                conn.commit()
