...
class ProveedorPersonaNaturalDB:
    def GetItems():
        try:
            conn = get_connection()
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                cursor.callproc("sp_ProveedorPersonaNaturalAllItems")
                resulset = cursor.fetchall()
            conn.close()
            list = []

            for row in resulset:
                Data_ent = ProveedorPersonaNaturalItemModel.Cargar(row)
    ...
    def GetItem(Id: int):
        try:
            ...
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                args = (Id,)
                cursor.callproc("sp_ProveedorPersonaNaturalAllItem", args)
                resulset = cursor.fetchall()
            conn.close()
            list = []
    ...
    def Save(Ent: ProveedorPersonaNaturalSaveModel):
        try:
            Store: str
            Store = "sp_ProveedorPersonaNatural_Save"
            ...
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                args = []
                args.append(Ent.ProveedorPersonaNaturalId)
                args.append(Ent.ProveedorId)
                args.append(Ent.PersonaNaturalId)
                cursor.callproc(Store, args)
                Ent.ProveedorPersonaNaturalId = int(cursor.fetchone()["v_ProveedorPersonaNaturalId"])
    ...
    def Delete(Id: int):
        try:
            ...
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                args = (Id,)
                cursor.callproc("sp_ProveedorPersonaNatural_Delete", args)
                conn.commit()
