    ...
    def GetItems():
        try:
            ...
            with conn.cursor() as cursor:
                ...
                cursor.callproc("sp_DepartamentoAllItems")
                resulset = cursor.fetchall()
            conn.close()
            list = []

            for row in resulset:
                Data_ent = DepartamentoItemModel.Cargar(row)
    ...
    def GetItem(Id: int):
        try:
            ...
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                args = (Id,)
                cursor.callproc("sp_DepartamentoAllItem", args)
                resulset = cursor.fetchall()
            conn.close()
            list = []
    ...
    def Save(Ent: DepartamentoSaveModel):
        try:
            ...
            with conn.cursor() as cursor:
                ...
                args.append(Ent.CodDepartamento)
                args.append(Ent.NomDepartamento)
                cursor.callproc(Store, args)
                Ent.DepartamentoId = int(cursor.fetchone()["v_DepartamentoId"])
    ...
    def Delete(Id: int):
        try:
            ...
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                args = (Id,)
                cursor.callproc("sp_Departamento_Delete", args)
                conn.commit()
