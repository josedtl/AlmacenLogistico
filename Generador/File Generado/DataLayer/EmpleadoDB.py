    ...
    def GetItems():
        try:
            ...
            with conn.cursor() as cursor:
                ...
                cursor.callproc("sp_EmpleadoAllItems")
                resulset = cursor.fetchall()
            conn.close()
            list = []

            for row in resulset:
                Data_ent = EmpleadoItemModel.Cargar(row)
    ...
    def GetItem(Id: int):
        try:
            ...
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                args = (Id,)
                cursor.callproc("sp_EmpleadoAllItem", args)
                resulset = cursor.fetchall()
            conn.close()
            list = []
    ...
    def Save(Ent: EmpleadoSaveModel):
        try:
            ...
            with conn.cursor() as cursor:
                ...
                args.append(Ent.CargoId)
                args.append(Ent.CorreoCorporativo)
                args.append(Ent.FechaIngreso)
                args.append(Ent.FechaRegistro)
                args.append(Ent.CodUsuario)
                args.append(Ent.Estado)
                cursor.callproc(Store, args)
                Ent.EmpleadoId = int(cursor.fetchone()["v_EmpleadoId"])
    ...
    def Delete(Id: int):
        try:
            ...
            with conn.cursor() as cursor:
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                args = (Id,)
                cursor.callproc("sp_Empleado_Delete", args)
                conn.commit()
